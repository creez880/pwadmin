...
# Find all directories to construct a dynamic classpath
...
echo "Compiling all Java files under $WEB_INF_DIR..."
javac -d "$OUTPUT_DIR" -cp "$CLASSPATH" $JAVA_FILES

# Check the compilation result
if [ $? -eq 0 ]; then
  echo "Compilation successful. Classes are in $OUTPUT_DIR."
else
